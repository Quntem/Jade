...
The agent also owns its WireGuard keypair locally. It reports only the public
key to Jade and stores dry-run VPN configs under `~/.jade/vpn` by default.
Override the dry-run config directory with `JADE_VPN_CONFIG_DIR=/path/to/dir`.
